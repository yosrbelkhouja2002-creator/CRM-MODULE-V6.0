...
class CRMAPI(http.Controller):
    ...
    # =====================================================
    # 🔹 POST : BULK CREATE LEADS (n8n → Odoo)
    # =====================================================
    @http.route(
    ...
    def bulk_create_leads(self, **kw):
        """Crée des leads dans crm.lead avec tous les champs"""
                ...
                try:
                    ...
                    if item.get('contact_name') or item.get('email_from'):
                        contact_vals = {
                            ...
                            'is_company': False,  # C'est une personne physique
                            'type': 'contact',    # Type de contact
                        }
                        contact_partner = env['res.partner'].sudo().create(contact_vals)
                        _logger.info(f"Contact créé : ID {contact_partner.id} - {contact_partner.name}")
                    ...
                    lead_vals = {
                        ...
                        'business_unit_id': item.get('business_unit_id'),  # ✅ Business Unit
                        ...
                        'Mode_de_livraison': item.get('Mode_de_livraison'),  # ✅ Mode de livraison
                    }

                    # ✅ Créer dans crm.lead
                    lead = env['crm.lead'].sudo().create(lead_vals)
                    _logger.info(f"Lead créé : ID {lead.id} - {lead.name}")
                    ...
                    _logger.exception(f"Erreur création lead : {lead_error}")
                    return request.make_response(
